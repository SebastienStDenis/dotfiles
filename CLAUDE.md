...
- **`agents/AGENTS.md` is the one set of agent instructions**, symlinked to `~/.config/agents/AGENTS.md` (the `AGENTS_FILE` variable in the Makefile). That XDG path is the hub every agent resolves to; no tool defines it, so it stays tool-neutral and out of `$HOME`. Each agent's own global file points at the hub rather than repeating it: `claude/CLAUDE.md` is a single `@~/.config/agents/AGENTS.md` import, `opencode/opencode.jsonc` lists the same path under `instructions`, and Codex - which has no import syntax - gets `~/.codex/AGENTS.md` symlinked to the hub, so it resolves through the same file instead of reaching into the repo separately. Tool-specific rules go in the pointer file; anything that applies everywhere goes in `agents/AGENTS.md`.
