...
- **No target installs an agent CLI.** `claude-setup` requires `claude` on PATH and fails fast; `opencode-setup` only links the config, so it is harmless on a machine without opencode. opencode's own installer drops the binary in `~/.opencode/bin` rather than a directory already on PATH, which is why `zsh/.zshrc` adds that directory when it exists.
