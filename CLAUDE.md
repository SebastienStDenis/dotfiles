...
Dotfiles for macOS and Linux, managed by a single Makefile. Each top-level directory holds one tool's config (`brew/`, `claude/`, `cursor/`, `git/`, `iterm2/`, `linearmouse/`, `rcmd/`, `zsh/`), and `link-*` targets symlink those files into `$HOME`. macOS gets the full setup; Linux gets Homebrew, git, zsh, and Claude (GUI apps and their configs are macOS-only). There is no build, lint, or test suite; verification is running the relevant make target (or `make -n <target>` to dry-run).

## Commands

- `make help` - list all targets with descriptions
- `make bootstrap-mac` - full macOS setup: Homebrew, packages, git, Oh My Zsh, iTerm2, Cursor, Claude
- `make bootstrap-linux` - Linux setup: Homebrew, packages, git, Oh My Zsh, and Claude
...
- **One Brewfile for both OSes.** `brew bundle` on Linux installs the formulas and skips unsupported casks automatically. Dump targets are macOS-only (enforced by the `mac_only` guard): a dump on Linux would rewrite the Brewfile without the mac casks.
