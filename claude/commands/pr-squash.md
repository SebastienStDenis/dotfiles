---
description: Rebase, squash, validate, push, and create a PR with user checkpoints at every destructive step.
allowed-tools: Bash, Read
---

# pr-squash

Automate the full workflow of rebasing, squashing, validating, pushing, and opening a GitHub PR.
...
If the current branch IS the default branch, stop and tell the user.

Confirm `gh` CLI is installed and authenticated (`gh auth status`). If not, stop and tell the user.

Capture the merge base now, since later steps need it:
```
git merge-base HEAD origin/<default-branch>
```
Remember this commit SHA as `<merge-base>`.
...
Confirm the working tree and index are clean before resetting:
```
git status --porcelain
```
If non-empty, stop and report to the user.

Collect the pre-squash commit messages to seed the squashed commit message:
```
git log <merge-base>..HEAD --format='%s%n%n%b'
```

Soft-reset to the merge base captured in step 1 (not the tip of the default branch — they can differ if the default branch advanced during the rebase):
```
git reset --soft <merge-base>
```

All changes are now staged. Draft a single commit message in conventional-commits format, derived from the log output above. Present it to the user before committing. Ask if they want to adjust it, then commit.
...
After a rebase + squash, the local branch has diverged from the remote, so a plain `git push` will be rejected. Use:
```
